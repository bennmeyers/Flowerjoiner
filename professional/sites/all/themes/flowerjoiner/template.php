 ...
 */
function flowerjoiner_preprocess_maintenance_page(&$vars) {
  if (class_exists('Database', FALSE)) {
    flowerjoiner_preprocess_html($vars);  // set html vars (html.tpl.php is in maintenance-page.tpl.php)
    flowerjoiner_preprocess_page($vars);  // set page vars
  }
}
 ...
 * HTML preprocessing
 */
function flowerjoiner_preprocess_html(&$vars) {
  global $theme_key, $user;

// Add variables and paths needed for HTML5 and responsive support.
  $vars['base_path'] = base_path();
  $vars['path_to_flowerjoiner'] = drupal_get_path('theme', 'flowerjoiner');
...
// Set grids responsive stylesheets
  drupal_add_css(drupal_get_path('theme','flowerjoiner').'/css/drupal/drupal-system-min.css', array('group' => CSS_SYSTEM, 'every_page' => TRUE, 'weight' => -1));
  drupal_add_css(drupal_get_path('theme','flowerjoiner').'/css/drupal/drupal-default-min.css', array('group' => CSS_DEFAULT, 'every_page' => TRUE, 'weight' => -1));
  ...
  if(theme_get_setting('css_zone')) {
    ...
    drupal_add_css(drupal_get_path('theme','flowerjoiner').'/css/yui/pure-min.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => -2));
  }
  if(theme_get_setting('grid_responsive') == '1') {
    if(theme_get_setting('css_zone')) {
      ...
      drupal_add_css(drupal_get_path('theme','flowerjoiner').'/css/yui/grids-responsive-old-ie-min.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'IE 8', '!IE' => FALSE), 'every_page' => TRUE, 'weight' => -1));
      drupal_add_css(drupal_get_path('theme','flowerjoiner').'/css/yui/grids-responsive-min.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'gt IE 8'), 'every_page' => TRUE, 'weight' => -1));
    }
  }
global $language;
...
if(theme_get_setting('headerimg')) {
  ...
  if($lang_dir == 'rtl') {
    drupal_add_css(drupal_get_path('theme','flowerjoiner').'/_custom/headerimg-rtl/rotate.php', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => 2, 'preprocess' => FALSE));
  } else {
...
// Get css styles
function get_flowerjoiner_style() {
 ...
 */
function flowerjoiner_preprocess_username(&$vars) {
 ...
 * HTML processing
 */
function flowerjoiner_process_html(&$vars) {
 ...
 * Page preprocessing
 */
function flowerjoiner_preprocess_page(&$vars) {
 ...
 * Breadcrumb override
 */
function flowerjoiner_breadcrumb($vars) {
 ...
 * Block preprocessing
 */
function flowerjoiner_preprocess_block(&$vars) {
 ...
 * Node preprocessing
 */
function flowerjoiner_preprocess_node(&$vars) {
 ...
 */
function flowerjoiner_preprocess_comment(&$vars) {
 ...
 */
function flowerjoiner_preprocess_views_view(&$vars) {
 ...
 */
function flowerjoiner_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function flowerjoiner_image($vars) {
 ...
 */
function flowerjoiner_form($vars) {
...
function flowerjoiner_button($vars) {
  $element = $vars ['element'];
...
function flowerjoiner_image_button($vars) {
  $element = $vars ['element'];
 ...
 * Theme's pager
 */
function flowerjoiner_item_list($vars){
...
function flowerjoiner_pager_link($vars){
 ...
 */
/*
function flowerjoiner_menu_tree($vars) {
...
*/
function flowerjoiner_links__system_main_menu($vars, $is_child=false){

  $vars['#attributes']['class'][] = $is_child ? 'pure-menu-children': 'nav navbar-nav';
    ...
    if(empty($link['#title']))
      ...
      if(!empty($link['#below'])){
        if(theme_get_setting('dropdown') == '1') {
          $html .= '<li class="pure-menu-has-children pure-menu-allow-hover">';
        }
        ...
        if(theme_get_setting('dropdown') == '1') {
          $html .= flowerjoiner_links__system_main_menu(array('links' => $link['#below']), true);
        }
        $html .= '</li>';
      }
      else
        $html .= '<li>'.l($link['#title'], $link['#href'], array('attributes' => $link['#attributes'])).'</li>';
    }
  }
...
function flowerjoiner_css_alter(&$css) {
  $exclude = array(
  // drupal-system.css
