$(document).ready(function(){
	jQuery.get("/assets/xml/projects.xml",{},function(xml) { // insert projects
		...
		$('personal project',xml).each(function() { // insert personal projects
			var lsWebsite = $(this).find("website").text(); // put the site name in a variable
			var lsLink = $(this).find("link").text(); // put the link in a variable
			var lsDemoLink = $(this).find("demolink").text(); // put the link in a variable
			var lsDescription = $(this).find("description").text(); // put the description in a variable
			var lsThumb = $(this).find("thumbnail").text(); // put the thumbnail in a variable
			var lsID = lsWebsite.toLowerCase(); // reformat the string to be lower case
			var lsProject = '<div class="design_example"><p><a href="/assets/images/samples/full/' + lsThumb + '.jpg" class="lightbox" target="_blank" title="' + lsSection + '"><img src="/assets/images/samples/preview/' + lsThumb + '.png" alt="' + lsWebsite + '" /></a></p><p class="description"><strong><a href="' + lsLink + '" title="' + lsWebsite + '" rel="external">' + lsWebsite + '</a></strong> '; // define the Local String for the Project
			...
			lsID = lsID.replace(/\//g,"_"); // reformat the string to remove slashes
			lsID = lsID.replace(/ /g,"_"); // reformat the string to remove spaces
			$('#personal').append(lsProject); // append the projects together
		}); // each
